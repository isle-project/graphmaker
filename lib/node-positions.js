...
const meetEqualityConstraints = ( uT, vT, lambda, b ) => {
    ...
    const n = vT[0].length;
    const scaledUTb = mvMult(uT, b).map( (y, i) => (Math.abs(lambda[i]) <= ZERO_TOLERANCE) ? 0 : y / lambda[i] );
...
const feasibleAdjustLeq = ({ n, c, s, initial, nullspace, constraints, rhs, shift }) => {
    if ( n - c >= s ) {
        ...
        while ( lambda.every( lam => Math.abs(lam) < ZERO_TOLERANCE ) && shift > ZERO_TOLERANCE ) {
            shift /= 2;
        ...
        const redundancy = lambda.filter( lam => Math.abs(lam) < ZERO_TOLERANCE ).length;
        ...
        let recon = mvMult( transpose(vT), mvMult( uT, y ).map( (x, i) => Math.abs(lambda[i]) > ZERO_TOLERANCE ? x/lambda[i] : 0 ) );
        if ( c > 0 ) {
...
const constrainedEquilibrate = (positions, equality = UNCONSTRAINED, leq = UNCONSTRAINED, options = {}) => {
    ...
    while ( maxForce > tol && iter < max_iter ) {
        ...
        if ( haveLeq ) {
            ...
            if ( Math.abs(delta) < ZERO_TOLERANCE ) {
...
const parentPosConstraints = ( graph, nodesInOrder, orientation ) => {
    ...
    for ( let i = 0; i < nodesInOrder.length; ++i ) {
        ...
        for ( const child of children ) {
            ...
            if ( edge.target !== node ) {
                adj.addEdge( i, nodesInOrder.indexOf( edge.target ) );
            }
        }
    }
        ...
        for ( const edgeName of edges ) {
            ...
            if (target.target !== node ) {
                ...
                if ( children.length > 1 && isTree ) {
                    ...
                    extra.push( `'${node}'.${orientationDim} ${orientedOp} '${target.target}'.${orientationDim}` );
                }
            }
        }
    ...
    debug( 'Parent constraints added:' );
    debug( extra );
    const { equality: extraEq, leq: extraLeq } = convertConstraints( extra, nodesInOrder, graph.nodes, 0.4 /* 5 / (nodesInOrder.length + 1) */ );
...
const nodePositions = ( graph, options = {} ) => {
    ...
    const { equality, leq } = convertConstraints( constraints, nodesInOrder, nodes, 2 / Math.max(1, nodesInOrder.length) /* 2 * sigma / (1 + nodesInOrder.length) */ );
    ...
    // Running the constrained equilibration twice seems to help with the stability of the solution:
    const pos0 = constrainedEquilibrate( initialPositions, equality, leq, options );
    return constrainedEquilibrate( pos0.positions, equality, leq, options );
};
...
function initializeTreePositions( graph, orderedDim, orderedDir, sigma ) {
    ...
    for ( const node of allNodes ) {
        childrenOf[node] = graph._adjacency[node].map( edgeName => graph.edges[ edgeName ].target );
