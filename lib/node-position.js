...
const nodePositions2 = ( graph, options = {} ) => {
    const nodes = graph.nodes;
    const nodesInOrder = objectKeys( nodes );
    const sigma = options.sigma || 2;
    const hasParents = options.hasParents || false;
    const orientation = options.orientation || 'auto';
    const { equality, leq } = convertConstraints( graph.constraints, nodesInOrder );
    ...
    if ( hasParents ) {
        ...
        if ( hasExtra ) {
            ...
            equality.rhs = equality.matrix.concat( extraEq.rhs );
            leq.matrix = leq.matrix.concat( extraLeq.matrix );
            leq.rhs = leq.matrix.concat( extraLeq.rhs );
