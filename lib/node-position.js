...
const parentPosConstraints = ( graph, nodesInOrder, orientation ) => {
    ...
    for ( let i = 0; i < nodesInOrder.length; ++i ) {
        ...
        const children = graph.adjacency[node];
        for ( const child of children ) {
            if ( !seenEdge.has( child.edge ) && child.targetNode !== node ) {
                seenEdge.add( child.edge );
                adj.addEdge( i, nodesInOrder.indexOf( child.targetNode ) );
            }
        }
    }
    ...
    const extra = [];
    seenEdge.clear();
    console.log( 'orientation', orientationDim, orientationDir, orthogonalDim, orientedOp );
    ...
    for ( const nodeIndex of topoSort ) {
        ...
        for ( const target of edges ) {
            if ( !seenEdge.has( target.edge ) ) {
                seenEdge.add( target.edge );
...
const nodePositions = ( graph, options = {} ) => {
    ...
    if ( presentStyle === 'tree' ) {
        ...
        initialPositions = initializeGraphPositions( nodes, nodesInOrder, orderedDim, orderedDir, sigma );
    }
...
function initializeTreePositions( graph, orderedDim, orderedDir, sigma ) {
    ...
    for ( const node of allNodes ) {
        childrenOf[node] = graph.adjacency[node].filter( edgeObj => !edgeSeen.has( edgeObj.edge ) ).map( edgeObj => {
            edgeSeen.add( edgeObj.edge );
            return edgeObj.targetNode;
        });
    ...
    const orderStatistics = nodesInOrder.filter( key => nodes[key]?.[orderedDim] === void 0 ).map( _ => randomNormal(0, sigma) );
    orderStatistics.sort( (a, b) => orderedDir * (a - b) );
    let statIndex = 0;
    const orderedCoord = nodesInOrder.map( key =>
        nodes[key]?.[orderedDim] === void 0 ? orderStatistics[statIndex++] : nodes[key][orderedDim]
    );
    ...
    const positions = nodesInOrder.map( (key, index) => {
        if ( orderedDim === 'x' ) {
            return [
                nodes[key]?.x === void 0 ? orthogonalCoord[levels[index]] : nodes[key].x,
                orderedCoord[ index ]
            ];
        }
        return [
            orderedCoord[ index ],
            nodes[key]?.y === void 0 ? orthogonalCoord[levels[index]] : nodes[key].y
         ];
    } );
...
function initializeGraphPositions( nodes, nodesInOrder, orderedDim, orderedDir, sigma ) {
    const orderedCoord = nodesInOrder.map( key =>
        nodes[key]?.[orderedDim] === void 0 ? randomNormal(0, sigma) : nodes[key][orderedDim]
    );
