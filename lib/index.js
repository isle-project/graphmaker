...
async function updateGraphSet( state, payload ) {
    ...
    newHistory.push( { diff, graph: newGraph, payload: '', action: null } );
    ...
    for ( const key in changes ) {
        if ( hasOwnProp( changes, key ) ) {
            deepSet(
                ...
                true,
                true,
                (changes[ key ] === null) ? void 0 : changes[ key ]
            );
        }
    }
    return outGraph;
...
async function updateGraphTask( state, payload ) {
    ...
    const newHistory = state.history
        .slice(0, state.historyIndex + 1);
    newHistory.push( { diff: graphDiff, graph: newGraph, payload: payload.prompt, action: 'TASK' });
    const newState = {
        graph: newGraph,
        history: newHistory,
        historyIndex:
