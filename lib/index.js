...
const { addSvgFromString } = require( '@yet3/svg2jspdf' );
 ...
 *
 * @param state - the current state of the graph
 * @param payload.format - format to save the graph in; one of 'png', 'svg', 'json' (default: 'svg')
 * @param payload.draft - whether to save the graph in draft mode (default: false)
 */
async function updateGraphSave( state, { format, draft } = { format: 'json', draft: false } ) {
    const populatedGraph = deepCopy( state.graph );
    ...
    if ( format === 'svg' ) {
        ...
        try {
            ...
            const doc = new jsPDF({ // eslint-disable-line new-cap
                format: [
                    state.graph.width || 500,
                    state.graph.height || 500
                ]
            }); // eslint-disable-line new-cap
            addSvgFromString( doc, svg );
            const out = Buffer.from( await doc.output( 'arraybuffer' ) );
        ...
        // TODO: Implement TikZ output
        throw new Error( 'TikZ output not yet implemented.' );
    }
    else if ( format === 'json' ) {
