...
function jsonRefReplacer( key, value ) {
    ...
    // If description in schema is an object, it will have a 'human' and 'model'
    // field, and we only send the 'model' field to the model. If it is just a
    // string, then just send that. If an object but no 'model' field, send
    // no description, not even an empty string.
    if ( key === UNIQUE + 'description' && typeof value === 'object' ) {
        return value[UNIQUE+'model'] || void 0;
    }
    return value;
}
