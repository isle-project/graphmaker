 ...
 *
 * @param {(string|Object)} diffSpec - a JSON string or object representing the changes in the graph
 * @returns {Function} a function that generates a text summary of a graph or a graph component
 */
function autoSummarizer( diffSpec ) {
    const changes = isString(diffSpec) ? JSON.parse( diffSpec ) : diffSpec;
...
function summarizeGraph( graph ) {
    ...
    const out = [];
    const icon = graph.directed ? '->' : '--';
    const adjList = {};
    let isSimple = true;
    let hasSelfLoops = false;

    // Record uni-directional edges by name and classify graph
    for ( const node of nodeNames ) {
        const seenTargets = {};
        adjList[node] = [];
        for ( const edgeName of adjacency[node] ) {
            const target = graph.edges[edgeName].target;
            adjList[node].push( target );
            if ( target === node ) {
                hasSelfLoops = true;
            }
            if ( isSimple ) {
    ...
    // Symmetrize adjacency list for undirected graphs
    if ( !graph.directed ) {
        for ( const node of nodeNames ) {
    ...
    /* Save temporarily until logic check on above nicer version
    if ( graph.directed ) {
        icon = '->';
        for ( const node of nodeNames ) {
            const seenTargets = {};
            adjList[node] = [];
            adjList[node] = adjacency[node].forEach( edgeName => {
                const target = graph.edges[edgeName].target;
                adjList[node].push( target );
                if ( target === node ) {
                    hasSelfLoops = true;
        ...
        for ( const node of nodeNames ) {
            const seenTargets = {};
            adjacency[node].forEach( edgeName => {
                const target = graph.edges[edgeName].target;
                adjList[node].push( target );
                if ( target === node ) {
                    hasSelfLoops = true;
    ...
    */
    ...
    const classification = {
        directed: graph.directed ? 'a directed' : 'an undirected',
        simple: isSimple ? 'simple graph' : 'multi-graph',
        self: hasSelfLoops ? ' with self loops' : ''
    };
    out.push( `This is ${classification.directed}, ${classification.simple}${classification.self}.` );
    return out.join('\n');
}
