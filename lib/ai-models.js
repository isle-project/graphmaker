...
add an edge from A to B => {"edges.AB":{"sourceNode":"A","targetNode":"B"}}
make the graph directed => {"directed":true}
add a directed edge from B to C => {"directed":true,"edges.BC":{"sourceNode":"B","targetNode":"C"}}
...
add two distinct edges from node C to node E => {"edges.CE1":{"sourceNode":"C","targetNode":"E"},"edges.CE2":{"sourceNode":"C","targetNode":"E"}}
...
async function streamedChatCompletion( model, messages ) {
    ...
    for ( const message of messages ) {
        nTokens += tokenizer.encode( message.content ).length;
    }
        ...
        case 'openai':
            if ( !model ) {
                model = 'gpt-3.5-turbo';
                      ...
                      // console.log( 'TOKENS:', tokenizer.encode( 'Hello World!' ).length )
            if ( !type ) {
                type = 'serial';
            }
            if ( type === 'memoryless' ) {
