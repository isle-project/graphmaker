...
function assembleChatHistory( userPrompt, graph, history, maxLookBack = 3 ) {
    ...
    if ( history.length <= 1 ) {
        ...
        const idx = Math.max( history.length - maxLookBack, 1 );
        const base = (history.length > maxLookBack) ? history[idx - 1].graph : baseGraph();
        out.push({
            role: 'user',
            content: assemblePrompt( base, history[idx].prompt )
        });
