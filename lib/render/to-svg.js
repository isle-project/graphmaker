 ...
 * @param {Array} position - (x,y) coordinates of the node
...
async function toSVG( graph, positions, config = {} ) {
    ...
    for ( let i = 0; i < adjacencyKeys.length; ++i ) {
        ...
        let selfLoopAngles = [];
        if ( nSelfLoops > 0 ) {
            const hist = directionalHistogram( sourceIndex, positions, DIRECTIONAL_BINWIDTH );
            selfLoopAngles = selfLoopDirections( hist, nSelfLoops );
            if ( selfLoopAngles === null ) {  // Edge case: no clear directions
                // Choose the nSelfLoops directionns with minimal histogram values
