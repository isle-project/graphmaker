...
async function toSVG( graph, positions, config = {} ) {
    ...
    for ( let i = 0; i < adjacencyKeys.length; ++i ) {
        ...
        for ( let edgeName of adjacency[source] ) {
            ...
            if ( edge === void 0 || edge === '' ) {
                edge = source + edge.targetNode;
            }
            if ( edgesSeen.has( edgeName ) ) {
                continue;
            }
            edgesSeen.add( edgeName );
            const edgeObject = edges[edgeName];
            console.log( 'edgeObject:', edgeObject );
            const targetIndex = nodeKeys.indexOf( edge.targetNode );
            ...
            if ( edge.targetNode === source ) { // Case: Self Loop
                drawSelfLoopEdge({
                    pos: sourcePos,
                    edge: edgeObject,
                    key: edgeName,
