...
const { circle, diamond, line, ellipse, rectangle, square, triangle, arrowhead, text, rotatedText, bezier, doubleCircle } = require( './svg-elements.js' );
...
async function drawNode({ node, position, canvas, baseSize, key, isDraft }) {
    ...
    switch ( shape ) {
        case 'circle':
            ...
            break;
        case 'double-circle':
            doubleCircle( position[0], position[1], baseSize/2 * size, myStyles ).addTo( canvas );
            await drawNodeLabel({ node, position, canvas, key, hSize: baseSize/2, vSize: baseSize/2, isDraft });
            break;
        case 'square':
