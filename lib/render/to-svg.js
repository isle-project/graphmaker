...
const extractStyles = require( './extract-styles.js' );
...
function drawNode({ node, position, canvas, size, styles, key, isDraft }) {
    ...
    console.log( 'About to Project Style' );
    const myStyles = projectStyle( nodeStyle, node.shape );

    console.log( 'fillStyle: ', nodeStyle.fillStyle );
    if ( nodeStyle.fillStyle && nodeStyle.fillStyle !== 'solid' ) {
        console.log( 'Adding fill pattern to canvas: ', nodeStyle.fillStyle );
        // Ensure that any patterns used for fill are in the svg before their use
        console.log( 'URL:'+ myStyles.fill.url() );
        myStyles.fill.addTo( canvas );
        console.log( 'Added fill pattern to canvas...' );
        nodeStyle.fillStyle = 'solid';  // ATTN! HACK ALERT -- so label is drawn without a fillStyle
    }
    switch ( node.shape ) {
        case 'circle':
            circle( position[0], position[1], size/2 * node.size, myStyles ).addTo( canvas );
            drawNodeLabel({ node, position, canvas, key, nodeStyle, hSize: size/2, vSize: size/2, isDraft });
            break;
        case 'square':
            square( position[0], position[1], size * node.size, myStyles ).addTo( canvas );
            drawNodeLabel({ node, position, canvas, key, nodeStyle, hSize: size/2, vSize: size/2, isDraft });
            break;
        case 'rectangle':
            rectangle( position[0], position[1], size * node.size * node.widthToHeight, size * node.size, myStyles ).addTo( canvas );
            drawNodeLabel({ node, position, canvas, key, nodeStyle, hSize: node.widthToHeight * size/2, vSize: size/2, isDraft });
            break;
        case 'ellipse':
            ellipse( position[0], position[1], size * node.size * node.widthToHeight/2, size * node.size/2, myStyles ).addTo( canvas );
            drawNodeLabel({ node, position, canvas, key, nodeStyle, hSize: node.widthToHeight * size/2, vSize: size/2, isDraft });
            break;
        case 'diamond':
            diamond( position[0], position[1], size * node.size * node.widthToHeight, size * node.size, myStyles ).addTo( canvas );
            drawNodeLabel({ node, position, canvas, key, nodeStyle, hSize: node.widthToHeight * size/2, vSize: size/2, isDraft });
            break;
        case 'triangle':
            triangle( position[0], position[1], size/2 * node.size, myStyles ).addTo( canvas );
            drawNodeLabel({ node, position, canvas, key, nodeStyle, hSize: node.widthToHeight * size/2, vSize: size/2, isDraft });
            break;
        default:
...
function drawLinearEdge( { sourcePos, targetPos, edge, key, edgeStyle, size, arrow, canvas, isDraft } ) {
    const myStyles = projectStyle( edgeStyle, 'line' );
...
function toSVG( graph, config = {} ) {
    ...
    let { positions, converged, maxForce } = nodePositions( nodes, constraints, { ambient: 0.1 } );
    ...
    for ( let i = 0; i < adjacencyKeys.length; ++i ) {
        ...
        for ( const { targetNode, edge } of adjacency[source] ) {
            ...
            const edgeStyle = extractStyles( edgeObject, graph.styles );
