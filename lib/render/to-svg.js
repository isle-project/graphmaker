...
function toSVG( graph ) {
    ...
    for ( let i = 0; i < graph.decorations.length; ++i ) {
        ...
        switch ( decoration.payload.type ) {
            case 'text':
                text( decoration.x, decoration.y, 0, 0, decoration.payload.text, decorationStyle ).addTo( canvas );
                break;
            case 'region':
                rectangle( decoration.x, decoration.y, decoration.payload.region[0], decoration.payload.region[1], decorationStyle ).addTo( canvas );
                break;
            default:
                throw new Error( `Unrecognized decoration type: ${decoration.payload.type}` );
        }
    }
