 ...
 *
 * @param {Array} sourcePos - (x,y) coordinates of the source node
 * @param {Array} targetPos - (x,y) coordinates of the target node
 ...
 * @returns {Array} (x,y) coordinates of the point on the source node boundary along the given line
 */
function shapeBoundary( [sourceX, sourceY], [targetX, targetY], node, arrow, baseSize ) {
    ...
    switch ( node.nodeShape ) {
        ...
        case 'triangle':
            return triangleBoundary( [sourceX, sourceY], [targetX, targetY], width, height, arrow );
        default:
...
function rectangleBoundary( [sourceX, sourceY], [targetX, targetY], width, height, arrow ) {
    ...
    if ( arrow ) {
        ...
        const arrowWidth = arrow.attr('markerWidth');
        console.log( 'arrowHeight', arrowHeight, 'arrowWidth', arrowWidth );
        arrowShift = arrowHeight / totalLength;
    }
    console.log( 'deltaX', deltaX, 'deltaY', deltaY, 'neX', neX, 'neY', neY, 'neHalf', neHalf, 'seHalf', seHalf );
    ...
    console.log( 'Boundary before arrow shift: ' );
...
function ellipseBoundary( [sourceX, sourceY], [targetX, targetY], width, height, arrow ) {
    throw new Error( 'ellipseBoundary not yet supported' );
}

function diamondBoundary( [sourceX, sourceY], [targetX, targetY], width, height, arrow ) {
    throw new Error( 'diamondBoundary not yet supported' );
}

function triangleBoundary( [sourceX, sourceY], [targetX, targetY], width, height, arrow ) {
    throw new Error( 'triangle not implemented yet' );
}
...
function drawLaTeXLabel( label, x, y, style ) {
    const { children, attributes } = latexSvgScaled( label, x, y, style.xExtent, style.yExtent );
    console.log( 'Children: ', children );
    console.log( 'Attributes: ', attributes );
    // Create a G element to hold the children
    const g = new G( attributes );
...
function drawMathLingoLabel( label, x, y, style ) {
...
function drawNodeLabel({ node, position, canvas, key, hSize, vSize, isDraft }) {
    ...
    if ( node.label ) {
        ...
        if ( match ) {
            ...
            const handler = LABEL_HANDLERS[ handlerName ];
            if ( handler ) {
                labelStyle.xExtent = hSize;
                labelStyle.yExtent = vSize;
                const g = handler( label, posX, posY, labelStyle );
                if ( g ) {
                    g.addTo( canvas );
                }
                return;
...
function drawNode({ node, position, canvas, baseSize, key, isDraft }) {
    ...
    switch ( shape ) {
        ...
        case 'rectangle':
            ...
            drawNodeLabel({ node, position, canvas, key, hSize: node.nodeWidthToHeight * baseSize/2, vSize: baseSize/2, isDraft });
            break;
        case 'ellipse':
...
function drawLinearEdge( { sourcePos, targetPos, edge, key, baseSize, source, target, arrow, canvas, isDraft } ) {
    ...
    // const [x0, y0, x1, y1] = edgeExtent( sourcePos, targetPos, arrow, baseSize );
    console.log( 'Draw linear edge...' );
    console.log( 'sourcePos', sourcePos, 'targetPos', targetPos );
...
function drawSelfLoopEdge( { pos, edge, key, baseSize, source, arrow, canvas, isDraft, angleLeaving, angleEntering, softnessEntering = 0.95, softnessLeaving = 0.95 } ) {
    const [ x, y ] = pos;
    const arrowSize = arrow ? arrow.attr('markerHeight') : 0;
    const [ dx, dy ] = rotate2d( DEFAULT_LOOP_UNIT[0], DEFAULT_LOOP_UNIT[1], 10 );
    const [ x0, y0 ] = [x + baseSize*dx, y + baseSize*dy];
    const [ x1, y1 ] = [x + (baseSize + arrowSize)*dx, y - (baseSize + arrowSize)*dy];
    ...
    const cx0 = x0 + softnessLeaving * 3 * baseSize * ux;
    const cy0 = y0 + softnessLeaving * 3 * baseSize * uy;
    const cx1 = x1 + softnessEntering * 3 * baseSize * vx;
    const cy1 = y1 + softnessEntering * 3 * baseSize* vy;
    const myStyles = projectStyle( edge, 'line' );
    const curveElem = bezier( x0, y0, cx0, cy0, cx1, cy1, x1, y1, myStyles );
    curveElem.addTo( canvas );
    if ( arrow ) {
...
function toSVG( graph, positions, config = {} ) {
