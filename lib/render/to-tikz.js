...
function toTikZ( graph, config = {} ) {
    ...
    for ( let i = 0; i < keys.length; i++ ) {
        ...
        tikzNodes.push(defineNode( name, node, extractStyles(node, {})));
    }
    ...
    for ( let i = 0; i < adjacencyKeys.length; ++i ) {
        ...
        for ( const { targetNode, edge } of adjacency[source] ) {
            ...
            tikzEdges.push( edgeToTikZ( edges[ edge ], source, targetNode, extractStyles( edges[ edge ], {} ), useArrow ) );
        }
    }
    return `\\begin{tikzpicture}[${describeStyles(graph)}]\n${tikzNodes.join('\n')}\n${tikzEdges.join('\n')}\n\\end{tikzpicture}`;
}
...
function defineNode( name, node, nodeStyles ) {
    ...
    const shape = asTikzShape( node.shape );
