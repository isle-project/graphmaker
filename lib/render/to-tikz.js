...
const { colorAsRGB, latexColor } = require( './colors.js' );
...
function fontSpecs( styles ) {
    ...
    if ( styles.fontColor ) {
        let color = latexColor( styles.fontColor );
        let scheme = '';
        if ( color.startsWith( '{' ) ) {
            const [r, g, b] = colorAsRGB( styles.fontColor );
            color = `${roundn(r/255, -4)},${roundn(g/255, -4)},${roundn(b/255, -4)}`;
            scheme = '[rgb]';
        }
...
function tikzFillStyle( fillStyle, fillColor ) {
    if ( !fillStyle ) {
        if ( fillColor ) {
            return `fill=${latexColor(fillColor)}`;
        }
        return '';
    }
    if ( isString(fillStyle) ) {
        switch ( fillStyle ) {
            case 'solid':
                return fillColor ? `fill=${latexColor(fillColor)}` : '';
            case 'hashed':
    ...
    const color = fillColor ? `, pattern color=${latexColor(fillColor)}` : '';
...
function tikzStyleDef( name, attributes ) {
    ...
    'z'
    */
    if ( attributes.lineColor ) styles.push( styleString('draw', latexColor(attributes.lineColor) ) );
    ...
    if ( attributes.fontColor ) styles.push( styleString('text', latexColor(attributes.fontColor) ) );
    if ( attributes.fillStyle || attributes.fillColor) {
        styles.push( tikzFillStyle(attributes.fillStyle, latexColor(attributes.fillColor)) );
    }
...
function texifyLabel( label, styles, { tangent, unit, drawAbove, extraOffset = [0, 0] } = {} ) {
    ...
    if ( styles.fontColor ) {
        specs.color = latexColor(styles.fontColor);
    }
...
function defineNode( name, node ) {
    ...
    const lineColor = styleString( 'draw', latexColor(node.lineColor) );
