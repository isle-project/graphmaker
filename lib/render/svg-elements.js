const { Circle, Line, Marker, Rect, Ellipse, Text, Path, PathArray, Polygon, G } = require( '@svgdotjs/svg.js' );
...
/**
 * Returns a SVG object showing two concentric circles of nearly the same size.
 *
 * @param {number} cx - x-coordinate of the center
 * @param {number} cy - y-coordinate of the center
 * @param {number} r - radius of the outer circle
 * @param {Object} [myStyles={}] - styles object
 * @returns {Object} double circle object
 */
const doubleCircle = (cx, cy, r, myStyles = {}) => {
    debug( 'circleStyles:', myStyles );
    let rRatio = 0.9;
    if ( myStyles?.stroke.width ) {
        rRatio = (r - 4 * myStyles.stroke.width) / r;
    }
    ...
    if ( myStyles.fill ) {
        inner.fill( myStyles.fill );
    }
    outer.fill('none');
    if ( myStyles.stroke ) {
        inner.stroke( myStyles.stroke );
        outer.stroke( myStyles.stroke );
    }
    const group = new G();
    group.add(inner);
    group.add(outer);
