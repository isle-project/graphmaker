...
const { initializeGraph, updateGraph, redo, resetGraph, resetPositions, undo, convert, task, setGraph, aiFactory } = require( './index.js' );
...
const { summarize } = require( './graph-summary.js' );
 ...
 *
 * @param {Object} output - {kind, data} an object containing a list
 *    of error objects as data if kind === 'ERRORS'
...
async function replEval( cmd, callback, server, noCallback = false ) {
    ...
    switch ( command ) {
        case ':show': {
            const summaryFormats = [ 'graph', 'nodes', 'edges', 'decorations', 'constraints', 'data' ];
            if ( arg === void 0 ) {
                arg = graphConfig.format;
            }
            if ( arg === 'json' ) {
                [ replState, output ] = await updateGraph( replState, convert( { ...graphConfig, format: 'json', draft: true } ) );
                return callback( null, JSON.stringify( replState.graph, ( key, value ) => {
                    return key.startsWith( '_' ) ? void 0 : value;
                }, 2 ) );
            } else if ( arg === 'tex' || arg === 'latex' ) {
                [ replState, output ] = await updateGraph( replState, convert( { ...graphConfig, format: 'tex', draft: true } ) );
            ...
            if ( value === void 0 ) {
                ...
                switch ( optType ) {
                    ...
                    case 'maybe-string':
                        ...
                        } else if ( clOptions[ option ].enum && !clOptions[ option ].enum.includes( value ) ) {
                            return callback( null, `Invalid value for option ${optionLabel}: ${value}. Must be one of: ${clOptions[ option ].enum.join( ', ' )}.` );
                        }
                        break;
                    default:
        ...
        default:
            ...
            if ( !handleErrors( output, callback ) && !noCallback ) {
                server.setPrompt( replState.config.autoshow ? '' : PROMPT );
                callback( null, randomChoice(RESPONSES) );
                if ( replState.config.autoshow ) {
                    // Show the graph (as in :show with the autoshow format)
                    const textFormats = [ 'tex', 'latex', 'json', 'graph', 'nodes', 'edges', 'decorations', 'constraints', 'data' ];
                    await replEval( `:show ${replState.config.autoshow}`, (_, out) => {
                        if ( textFormats.includes( replState.config.autoshow ) ) {
                            process.stdout.write('\n' + out + '\n');
                        }
                    }, server );
                }
