 ...
 *
 */
function autoshowText( replState, baseOutput='', { prefix='', beginHistoryIndex, endHistoryIndex } = {} ) {
    ...
    if ( replState.config.autoshow === 'auto' ) {
        const indices = (beginHistoryIndex !== void 0 || endHistoryIndex !== void 0)   // endHistoryIndex is exclusive
...
async function replEval( cmd, callback, server, noCallback = false ) {
    ...
    switch ( command ) {
        ...
        case ':exec': {
            ...
            callback( null, autoshowText( replState, `Executed commands in file ${file}.`, { beginHistoryIndex }) );
            break;
        }
        case ':load': {
