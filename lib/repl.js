...
async function replEval( cmd, callback, server, noCallback = false ) {
    ...
    switch ( command ) {
        ...
        case ':exec': {
            ...
            const batch = configExec === 'batch' || (configExec === 'auto' && DEFAULT_EXEC_POLICIES[model] === 'batch');
            ...
            file = file.trim();
            const RE_LINE_COMMENT = /^\s*(?:#.*)?$/mg;
            let execCmds = fs.readFileSync( file, 'utf8' ).replace( RE_LINE_COMMENT, '' );
            if ( batch ) {
                execCmds = execCmds.split( /^\s*(:.*\n)/m );
                execCmds[ 0 ] = prefix + '\n' + execCmds[ 0 ];
                execCmds[ execCmds.length - 1 ] = execCmds[ execCmds.length - 1 ] + '\n' + suffix;
            } else {
                execCmds = execCmds.split( /\s*(?:\r?\n)+\s*/m );
        ...
        case ':help':
            callback( null, 'Commands:\n' +
                ...
                '  :exec [prefix=\'...\'] [suffix=\'...\'] <filename> - load a file containing commands (preceded and followed by an optional prefix and suffix; one of the delimiters `,\', or " is required and must not appear in the string)\n' +
...
function filePathCompletion( arg ) {
    const noArg = !arg;
...
const replServer = repl.start({
    ...
    completer: ( line ) => {
        ...
        if ( line.startsWith( ':exec' ) ) {
            ...
            const { hits, prefix } = filePathCompletion( arg );
