...
const { summarize, autoSummarizer } = require( './graph-summary.js' );
 ...
 *
 */
function autoshowText( replState, baseOutput='', prefix='' ) {
    const textFormats = [ 'tex', 'latex', 'json', 'auto', 'graph', 'nodes', 'edges', 'decorations', 'constraints', 'data' ];
    const init = prefix && (prefix + '\n');
    let autoshow = '';
    if ( replState.config.autoshow === 'auto' ) {
        const summarizer = autoSummarizer( replState.history[ replState.historyIndex ].diff );
        autoshow = prefix + summarizer(replState.graph) + '\n';
    } else if ( replState.config.autoshow && textFormats.includes(replState.config.autoshow) ) {
        autoshow = prefix + summarize( replState.config.autoshow, replState.graph) + '\n';
    }
...
async function replEval( cmd, callback, server, noCallback = false ) {
    ...
    switch ( command ) {
        ...
        case ':exec': {
            ...
            callback( null, autoshowText( replState, `Executed commands in file ${file}.`) );
            break;
        }
        case ':load': {
        ...
        default:
            ...
            if ( !handleErrors( output, callback ) && !noCallback ) {
                callback( null, autoshowText( replState, randomChoice(RESPONSES)) );
            }
            break;
    }
}
